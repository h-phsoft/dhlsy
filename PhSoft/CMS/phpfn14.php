...
function CurrentProjectID() {
	...
	return "{EB80027D-BFC8-4F25-85BB-6B03A26BA4A8}";
}
	...
	function ValidateUser(&$usr, &$pwd, $autologin, $encrypted = FALSE, $provider = "") {
		...
		if (!$valid) {
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					$valid = $customValid || ew_ComparePassword($rs->fields('user_password'), $pwd, $encrypted);
					if ($valid) {
						$_SESSION[EW_SESSION_STATUS] = "login";
						$_SESSION[EW_SESSION_SYS_ADMIN] = 0; // Non System Administrator
						$this->setCurrentUserName($rs->fields('user_logon')); // Load user name

						// Call User Validated event
						$row = $rs->fields;
						$UserProfile->Assign($row);
						$UserProfile->Delete('user_password'); // Delete password
