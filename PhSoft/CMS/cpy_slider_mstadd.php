...
$cpy_slider_mst_add = NULL; // Initialize page object first

class ccpy_slider_mst_add extends ccpy_slider_mst {
	...
	// Page object name
	var $PageObjName = 'cpy_slider_mst_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["slid_id"] != "") {
				...
				$this->setKey("slid_id", $this->slid_id->CurrentValue); // Set up key
			} else {
				$this->setKey("slid_id", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$loaded) { // Record not loaded
					...
					$this->Page_Terminate("cpy_slider_mstlist.php"); // No matching record, return to list
				}
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "cpy_slider_mstview.php")
	...
	function ValidateForm() {
		...
		if (in_array("cpy_slider_trn", $DetailTblVar) && $GLOBALS["cpy_slider_trn"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("cpy_slider_trn", $DetailTblVar) && $GLOBALS["cpy_slider_trn"]->DetailAdd) {
				$GLOBALS["cpy_slider_trn"]->slid_id->setSessionValue($this->slid_id->CurrentValue); // Set master key
				if (!isset($GLOBALS["cpy_slider_trn_grid"])) $GLOBALS["cpy_slider_trn_grid"] = new ccpy_slider_trn_grid(); // Get detail page object
				$Security->LoadCurrentUserLevel($this->ProjectID . "cpy_slider_trn"); // Load user level of detail table
				$AddRow = $GLOBALS["cpy_slider_trn_grid"]->GridInsert();
				$Security->LoadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
				if (!$AddRow)
					$GLOBALS["cpy_slider_trn"]->slid_id->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetupDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("cpy_slider_trn", $DetailTblVar)) {
				...
				if ($GLOBALS["cpy_slider_trn_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["cpy_slider_trn_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["cpy_slider_trn_grid"]->CurrentMode = "add";
					$GLOBALS["cpy_slider_trn_grid"]->CurrentAction = "gridadd";
...
// Create page object
if (!isset($cpy_slider_mst_add)) $cpy_slider_mst_add = new ccpy_slider_mst_add();

// Page init
$cpy_slider_mst_add->Page_Init();

// Page main
$cpy_slider_mst_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$cpy_slider_mst_add->Page_Render();
?>
...
var CurrentForm = fcpy_slider_mstadd = new ew_Form("fcpy_slider_mstadd", "add");

// Validate form
fcpy_slider_mstadd.Validate = function() {
...
fcpy_slider_mstadd.Form_CustomValidate = 
...
fcpy_slider_mstadd.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php $cpy_slider_mst_add->ShowPageHeader(); ?>
<?php
$cpy_slider_mst_add->ShowMessage();
?>
<form name="fcpy_slider_mstadd" id="fcpy_slider_mstadd" class="<?php echo $cpy_slider_mst_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($cpy_slider_mst_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $cpy_slider_mst_add->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="cpy_slider_mst">
<input type="hidden" name="a_add" id="a_add" value="A">
<input type="hidden" name="modal" value="<?php echo intval($cpy_slider_mst_add->IsModal) ?>">
<div class="ewAddDiv"><!-- page* -->
<?php if ($cpy_slider_mst->slid_name->Visible) { // slid_name ?>
	<div id="r_slid_name" class="form-group">
		<label id="elh_cpy_slider_mst_slid_name" for="x_slid_name" class="<?php echo $cpy_slider_mst_add->LeftColumnClass ?>"><?php echo $cpy_slider_mst->slid_name->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $cpy_slider_mst_add->RightColumnClass ?>"><div<?php echo $cpy_slider_mst->slid_name->CellAttributes() ?>>
...
<?php if ($cpy_slider_mst->slid_rem->Visible) { // slid_rem ?>
	<div id="r_slid_rem" class="form-group">
		<label id="elh_cpy_slider_mst_slid_rem" for="x_slid_rem" class="<?php echo $cpy_slider_mst_add->LeftColumnClass ?>"><?php echo $cpy_slider_mst->slid_rem->FldCaption() ?></label>
		<div class="<?php echo $cpy_slider_mst_add->RightColumnClass ?>"><div<?php echo $cpy_slider_mst->slid_rem->CellAttributes() ?>>
<span id="el_cpy_slider_mst_slid_rem">
<input type="text" data-table="cpy_slider_mst" data-field="x_slid_rem" name="x_slid_rem" id="x_slid_rem" placeholder="<?php echo ew_HtmlEncode($cpy_slider_mst->slid_rem->getPlaceHolder()) ?>" value="<?php echo $cpy_slider_mst->slid_rem->EditValue ?>"<?php echo $cpy_slider_mst->slid_rem->EditAttributes() ?>>
</span>
...
<?php
	if (in_array("cpy_slider_trn", explode(",", $cpy_slider_mst->getCurrentDetailTable())) && $cpy_slider_trn->DetailAdd) {
?>
...
<?php } ?>
<?php if (!$cpy_slider_mst_add->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $cpy_slider_mst_add->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("AddBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $cpy_slider_mst_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<script type="text/javascript">
fcpy_slider_mstadd.Init();
</script>
<?php
$cpy_slider_mst_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$cpy_slider_mst_add->Page_Terminate();
?>
