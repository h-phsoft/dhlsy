...
$phs_perms_edit = NULL; // Initialize page object first

class cphs_perms_edit extends cphs_perms {
	...
	// Page object name
	var $PageObjName = 'phs_perms_edit';
	...
	function Page_Main() {
		...
		if ($this->TotalRecs <= 0) { // No record found
			...
			$this->Page_Terminate("phs_permslist.php"); // Return to list page
			...
			if (!is_null($this->perm_id->CurrentValue)) {
				while (!$this->Recordset->EOF) {
					if (strval($this->perm_id->CurrentValue) == strval($this->Recordset->fields('perm_id'))) {
	...
	function LoadFormValues() {
		...
		if (!$this->perm_id->FldIsDetailKey)
			$this->perm_id->setFormValue($objForm->GetValue("x_perm_id"));
	}
...
// Create page object
if (!isset($phs_perms_edit)) $phs_perms_edit = new cphs_perms_edit();

// Page init
$phs_perms_edit->Page_Init();

// Page main
$phs_perms_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$phs_perms_edit->Page_Render();
?>
...
var CurrentForm = fphs_permsedit = new ew_Form("fphs_permsedit", "edit");

// Validate form
fphs_permsedit.Validate = function() {
...
fphs_permsedit.Form_CustomValidate = 
...
fphs_permsedit.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fphs_permsedit.Lists["x_pgrp_id"] = {"LinkField":"x_pgrp_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_pgrp_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_pgroup"};
fphs_permsedit.Lists["x_pgrp_id"].Data = "<?php echo $phs_perms_edit->pgrp_id->LookupFilterQuery(FALSE, "edit") ?>";
...
<?php $phs_perms_edit->ShowPageHeader(); ?>
<?php
$phs_perms_edit->ShowMessage();
?>
<?php if (!$phs_perms_edit->IsModal) { ?>
<form name="ewPagerForm" class="form-horizontal ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($phs_perms_edit->Pager)) $phs_perms_edit->Pager = new cPrevNextPager($phs_perms_edit->StartRec, $phs_perms_edit->DisplayRecs, $phs_perms_edit->TotalRecs, $phs_perms_edit->AutoHidePager) ?>
<?php if ($phs_perms_edit->Pager->RecordCount > 0 && $phs_perms_edit->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($phs_perms_edit->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $phs_perms_edit->PageUrl() ?>start=<?php echo $phs_perms_edit->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($phs_perms_edit->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $phs_perms_edit->PageUrl() ?>start=<?php echo $phs_perms_edit->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $phs_perms_edit->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($phs_perms_edit->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $phs_perms_edit->PageUrl() ?>start=<?php echo $phs_perms_edit->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($phs_perms_edit->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $phs_perms_edit->PageUrl() ?>start=<?php echo $phs_perms_edit->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $phs_perms_edit->Pager->PageCount ?></span>
</div>
<?php } ?>
<div class="clearfix"></div>
</form>
<?php } ?>
<form name="fphs_permsedit" id="fphs_permsedit" class="<?php echo $phs_perms_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($phs_perms_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $phs_perms_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="phs_perms">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<input type="hidden" name="modal" value="<?php echo intval($phs_perms_edit->IsModal) ?>">
<div class="ewEditDiv"><!-- page* -->
<?php if ($phs_perms->pgrp_id->Visible) { // pgrp_id ?>
	<div id="r_pgrp_id" class="form-group">
		<label id="elh_phs_perms_pgrp_id" for="x_pgrp_id" class="<?php echo $phs_perms_edit->LeftColumnClass ?>"><?php echo $phs_perms->pgrp_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $phs_perms_edit->RightColumnClass ?>"><div<?php echo $phs_perms->pgrp_id->CellAttributes() ?>>
...
<?php if ($phs_perms->perm_table->Visible) { // perm_table ?>
	<div id="r_perm_table" class="form-group">
		<label id="elh_phs_perms_perm_table" for="x_perm_table" class="<?php echo $phs_perms_edit->LeftColumnClass ?>"><?php echo $phs_perms->perm_table->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $phs_perms_edit->RightColumnClass ?>"><div<?php echo $phs_perms->perm_table->CellAttributes() ?>>
...
<?php if ($phs_perms->perm_perm->Visible) { // perm_perm ?>
	<div id="r_perm_perm" class="form-group">
		<label id="elh_phs_perms_perm_perm" for="x_perm_perm" class="<?php echo $phs_perms_edit->LeftColumnClass ?>"><?php echo $phs_perms->perm_perm->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $phs_perms_edit->RightColumnClass ?>"><div<?php echo $phs_perms->perm_perm->CellAttributes() ?>>
...
<input type="hidden" data-table="phs_perms" data-field="x_perm_id" name="x_perm_id" id="x_perm_id" value="<?php echo ew_HtmlEncode($phs_perms->perm_id->CurrentValue) ?>">
<?php if (!$phs_perms_edit->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $phs_perms_edit->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $phs_perms_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
