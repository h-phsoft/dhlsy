...
$cpy_page_block_grid = NULL; // Initialize page object first

class ccpy_page_block_grid extends ccpy_page_block {
	...
	// Page object name
	var $PageObjName = 'cpy_page_block_grid';

	// Grid form hidden field names
	var $FormName = 'fcpy_page_blockgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["cpy_page_block"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->pblk_id->CurrentValue;
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->pblk_id->CurrentValue = strval($arKeys[0]); // pblk_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetupMasterParms() {
		...
		if ($sMasterTblVar == "cpy_page") {
			...
			if ($GLOBALS["cpy_page"]->EventCancelled) $this->EventCancelled = TRUE;
		}
