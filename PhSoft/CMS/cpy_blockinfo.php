...
//
// Table class for cpy_block
//
class ccpy_block extends cTable {
	var $blk_id;
	var $blk_name;
	var $blk_status;
	var $blk_type;
	...
	function __construct() {
		...
		$this->TableVar = 'cpy_block';
		$this->TableName = 'cpy_block';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`cpy_block`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// blk_id
		$this->blk_id = new cField('cpy_block', 'cpy_block', 'x_blk_id', 'blk_id', '`blk_id`', '`blk_id`', 3, -1, FALSE, '`blk_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// blk_name
		$this->blk_name = new cField('cpy_block', 'cpy_block', 'x_blk_name', 'blk_name', '`blk_name`', '`blk_name`', 200, -1, FALSE, '`blk_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->blk_name->Sortable = TRUE; // Allow sort
		$this->fields['blk_name'] = &$this->blk_name;

		// blk_status
		$this->blk_status = new cField('cpy_block', 'cpy_block', 'x_blk_status', 'blk_status', '`blk_status`', '`blk_status`', 2, -1, FALSE, '`blk_status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->blk_status->Sortable = TRUE; // Allow sort
		$this->blk_status->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->blk_status->PleaseSelectText = $Language->Phrase("PleaseSelect"); // PleaseSelect text
		$this->blk_status->OptionCount = 2;
		$this->blk_status->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['blk_status'] = &$this->blk_status;

		// blk_type
		$this->blk_type = new cField('cpy_block', 'cpy_block', 'x_blk_type', 'blk_type', '`blk_type`', '`blk_type`', 2, -1, FALSE, '`blk_type`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->blk_type->Sortable = TRUE; // Allow sort
		$this->blk_type->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->blk_type->PleaseSelectText = $Language->Phrase("PleaseSelect"); // PleaseSelect text
		$this->blk_type->OptionCount = 5;
		$this->blk_type->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['blk_type'] = &$this->blk_type;

		// blk_stext
		$this->blk_stext = new cField('cpy_block', 'cpy_block', 'x_blk_stext', 'blk_stext', '`blk_stext`', '`blk_stext`', 201, -1, FALSE, '`blk_stext`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
	...
	function GetDetailUrl() {
		...
		// Detail url
		$sDetailUrl = "";
		if ($this->getCurrentDetailTable() == "cpy_block_detail") {
			$sDetailUrl = $GLOBALS["cpy_block_detail"]->GetListUrl() . "?" . EW_TABLE_SHOW_MASTER . "=" . $this->TableVar;
			$sDetailUrl .= "&fk_blk_id=" . urlencode($this->blk_id->CurrentValue);
		}
		if ($sDetailUrl == "") {
			$sDetailUrl = "cpy_blocklist.php";
		}
		return $sDetailUrl;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`cpy_block`";
	}
	...
	function getSqlOrderBy() { // Order By
		return ($this->_SqlOrderBy <> "") ? $this->_SqlOrderBy : "`blk_type` DESC,`blk_name` ASC";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "cpy_blockedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "cpy_blockadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("cpy_blockview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("cpy_blockview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "cpy_blockadd.php?" . $this->UrlParm($parm);
		else
			$url = "cpy_blockadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("cpy_blockedit.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("cpy_blockedit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("cpy_blockadd.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("cpy_blockadd.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("cpy_blockdelete.php", $this->UrlParm());
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->blk_status->setDbValue($rs->fields('blk_status'));
		$this->blk_type->setDbValue($rs->fields('blk_type'));
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->blk_status->Exportable) $Doc->ExportCaption($this->blk_status);
					if ($this->blk_type->Exportable) $Doc->ExportCaption($this->blk_type);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->blk_status->Exportable) $Doc->ExportField($this->blk_status);
						if ($this->blk_type->Exportable) $Doc->ExportField($this->blk_type);
