...
$cpy_slider_trn_edit = NULL; // Initialize page object first

class ccpy_slider_trn_edit extends ccpy_slider_trn {
	...
	// Page object name
	var $PageObjName = 'cpy_slider_trn_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			...
			// Load key from Form
			if ($objForm->HasValue("x_tslid_id")) {
		...
		switch ($this->CurrentAction) {
			...
			Case "U": // Update
				...
				if (ew_GetPageName($sReturnUrl) == "cpy_slider_trnlist.php")
	...
	function LoadRowValues($rs = NULL) {
		...
		$this->slid_photo->CurrentValue = $this->slid_photo->Upload->DbValue;
	...
	function RenderRow() {
		...
		// slid_photo
		$this->slid_photo->UploadPath = '../../assets/img/mainSlider/';
		...
		$this->slid_text->ViewCustomAttributes = "";
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->slid_photo);
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$this->slid_photo->OldUploadPath = '../../assets/img/mainSlider/';
			...
			if ($this->slid_photo->Visible && !$this->slid_photo->Upload->KeepFile) {
				...
				if (!ew_Empty($this->slid_photo->Upload->Value)) {
					$rsnew['slid_photo'] = ew_UploadFileNameEx($this->slid_photo->PhysicalUploadPath(), $rsnew['slid_photo']); // Get new file name
				}
			}
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					if ($this->slid_photo->Visible && !$this->slid_photo->Upload->KeepFile) {
						if (!ew_Empty($this->slid_photo->Upload->Value)) {
							if (!$this->slid_photo->Upload->SaveToFile($rsnew['slid_photo'], TRUE)) {
...
// Create page object
if (!isset($cpy_slider_trn_edit)) $cpy_slider_trn_edit = new ccpy_slider_trn_edit();

// Page init
$cpy_slider_trn_edit->Page_Init();

// Page main
$cpy_slider_trn_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$cpy_slider_trn_edit->Page_Render();
?>
...
var CurrentForm = fcpy_slider_trnedit = new ew_Form("fcpy_slider_trnedit", "edit");

// Validate form
fcpy_slider_trnedit.Validate = function() {
...
fcpy_slider_trnedit.Form_CustomValidate = 
...
fcpy_slider_trnedit.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fcpy_slider_trnedit.Lists["x_slid_id"] = {"LinkField":"x_slid_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_slid_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"cpy_slider_mst"};
fcpy_slider_trnedit.Lists["x_slid_id"].Data = "<?php echo $cpy_slider_trn_edit->slid_id->LookupFilterQuery(FALSE, "edit") ?>";
...
<?php $cpy_slider_trn_edit->ShowPageHeader(); ?>
<?php
$cpy_slider_trn_edit->ShowMessage();
?>
<?php if (!$cpy_slider_trn_edit->IsModal) { ?>
<form name="ewPagerForm" class="form-horizontal ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($cpy_slider_trn_edit->Pager)) $cpy_slider_trn_edit->Pager = new cPrevNextPager($cpy_slider_trn_edit->StartRec, $cpy_slider_trn_edit->DisplayRecs, $cpy_slider_trn_edit->TotalRecs, $cpy_slider_trn_edit->AutoHidePager) ?>
<?php if ($cpy_slider_trn_edit->Pager->RecordCount > 0 && $cpy_slider_trn_edit->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($cpy_slider_trn_edit->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $cpy_slider_trn_edit->PageUrl() ?>start=<?php echo $cpy_slider_trn_edit->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($cpy_slider_trn_edit->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $cpy_slider_trn_edit->PageUrl() ?>start=<?php echo $cpy_slider_trn_edit->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $cpy_slider_trn_edit->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($cpy_slider_trn_edit->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $cpy_slider_trn_edit->PageUrl() ?>start=<?php echo $cpy_slider_trn_edit->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($cpy_slider_trn_edit->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $cpy_slider_trn_edit->PageUrl() ?>start=<?php echo $cpy_slider_trn_edit->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $cpy_slider_trn_edit->Pager->PageCount ?></span>
</div>
<?php } ?>
<div class="clearfix"></div>
</form>
<?php } ?>
<form name="fcpy_slider_trnedit" id="fcpy_slider_trnedit" class="<?php echo $cpy_slider_trn_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($cpy_slider_trn_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $cpy_slider_trn_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="cpy_slider_trn">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<input type="hidden" name="modal" value="<?php echo intval($cpy_slider_trn_edit->IsModal) ?>">
<?php if ($cpy_slider_trn->getCurrentMasterTable() == "cpy_slider_mst") { ?>
...
<?php if ($cpy_slider_trn->slid_id->Visible) { // slid_id ?>
	<div id="r_slid_id" class="form-group">
		<label id="elh_cpy_slider_trn_slid_id" for="x_slid_id" class="<?php echo $cpy_slider_trn_edit->LeftColumnClass ?>"><?php echo $cpy_slider_trn->slid_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $cpy_slider_trn_edit->RightColumnClass ?>"><div<?php echo $cpy_slider_trn->slid_id->CellAttributes() ?>>
...
<?php if ($cpy_slider_trn->slid_order->Visible) { // slid_order ?>
	<div id="r_slid_order" class="form-group">
		<label id="elh_cpy_slider_trn_slid_order" for="x_slid_order" class="<?php echo $cpy_slider_trn_edit->LeftColumnClass ?>"><?php echo $cpy_slider_trn->slid_order->FldCaption() ?></label>
		<div class="<?php echo $cpy_slider_trn_edit->RightColumnClass ?>"><div<?php echo $cpy_slider_trn->slid_order->CellAttributes() ?>>
...
<?php if ($cpy_slider_trn->slid_header->Visible) { // slid_header ?>
	<div id="r_slid_header" class="form-group">
		<label id="elh_cpy_slider_trn_slid_header" for="x_slid_header" class="<?php echo $cpy_slider_trn_edit->LeftColumnClass ?>"><?php echo $cpy_slider_trn->slid_header->FldCaption() ?></label>
		<div class="<?php echo $cpy_slider_trn_edit->RightColumnClass ?>"><div<?php echo $cpy_slider_trn->slid_header->CellAttributes() ?>>
...
<?php if ($cpy_slider_trn->slid_link->Visible) { // slid_link ?>
	<div id="r_slid_link" class="form-group">
		<label id="elh_cpy_slider_trn_slid_link" for="x_slid_link" class="<?php echo $cpy_slider_trn_edit->LeftColumnClass ?>"><?php echo $cpy_slider_trn->slid_link->FldCaption() ?></label>
		<div class="<?php echo $cpy_slider_trn_edit->RightColumnClass ?>"><div<?php echo $cpy_slider_trn->slid_link->CellAttributes() ?>>
<span id="el_cpy_slider_trn_slid_link">
<input type="text" data-table="cpy_slider_trn" data-field="x_slid_link" name="x_slid_link" id="x_slid_link" size="30" maxlength="250" placeholder="<?php echo ew_HtmlEncode($cpy_slider_trn->slid_link->getPlaceHolder()) ?>" value="<?php echo $cpy_slider_trn->slid_link->EditValue ?>"<?php echo $cpy_slider_trn->slid_link->EditAttributes() ?>>
</span>
...
<?php if ($cpy_slider_trn->slid_photo->Visible) { // slid_photo ?>
	<div id="r_slid_photo" class="form-group">
		<label id="elh_cpy_slider_trn_slid_photo" class="<?php echo $cpy_slider_trn_edit->LeftColumnClass ?>"><?php echo $cpy_slider_trn->slid_photo->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $cpy_slider_trn_edit->RightColumnClass ?>"><div<?php echo $cpy_slider_trn->slid_photo->CellAttributes() ?>>
<span id="el_cpy_slider_trn_slid_photo">
<div id="fd_x_slid_photo">
<span title="<?php echo $cpy_slider_trn->slid_photo->FldTitle() ? $cpy_slider_trn->slid_photo->FldTitle() : $Language->Phrase("ChooseFile") ?>" class="btn btn-default btn-sm fileinput-button ewTooltip<?php if ($cpy_slider_trn->slid_photo->ReadOnly || $cpy_slider_trn->slid_photo->Disabled) echo " hide"; ?>">
...
<?php if ($cpy_slider_trn->slid_text->Visible) { // slid_text ?>
	<div id="r_slid_text" class="form-group">
		<label id="elh_cpy_slider_trn_slid_text" class="<?php echo $cpy_slider_trn_edit->LeftColumnClass ?>"><?php echo $cpy_slider_trn->slid_text->FldCaption() ?></label>
		<div class="<?php echo $cpy_slider_trn_edit->RightColumnClass ?>"><div<?php echo $cpy_slider_trn->slid_text->CellAttributes() ?>>
<span id="el_cpy_slider_trn_slid_text">
<?php ew_AppendClass($cpy_slider_trn->slid_text->EditAttrs["class"], "editor"); ?>
<textarea data-table="cpy_slider_trn" data-field="x_slid_text" name="x_slid_text" id="x_slid_text" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($cpy_slider_trn->slid_text->getPlaceHolder()) ?>"<?php echo $cpy_slider_trn->slid_text->EditAttributes() ?>><?php echo $cpy_slider_trn->slid_text->EditValue ?></textarea>
<script type="text/javascript">
ew_CreateEditor("fcpy_slider_trnedit", "x_slid_text", 35, 4, <?php echo ($cpy_slider_trn->slid_text->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<input type="hidden" data-table="cpy_slider_trn" data-field="x_tslid_id" name="x_tslid_id" id="x_tslid_id" value="<?php echo ew_HtmlEncode($cpy_slider_trn->tslid_id->CurrentValue) ?>">
<?php if (!$cpy_slider_trn_edit->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $cpy_slider_trn_edit->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $cpy_slider_trn_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
...
<script type="text/javascript">
fcpy_slider_trnedit.Init();
</script>
<?php
$cpy_slider_trn_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$cpy_slider_trn_edit->Page_Terminate();
?>
