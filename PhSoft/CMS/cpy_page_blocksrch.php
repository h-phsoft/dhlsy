...
$cpy_page_block_search = NULL; // Initialize page object first

class ccpy_page_block_search extends ccpy_page_block {
	...
	// Page object name
	var $PageObjName = 'cpy_page_block_search';
			...
			switch ($this->CurrentAction) {
				case "S": // Get search criteria
					...
					if ($sSrchStr <> "") {
						...
						$sSrchStr = "cpy_page_blocklist.php" . "?" . $sSrchStr;
	...
	function BuildAdvancedSearch() {
		...
		$this->BuildSearchUrl($sSrchUrl, $this->pblk_order); // pblk_order
		$this->BuildSearchUrl($sSrchUrl, $this->pblk_status); // pblk_status
		$this->BuildSearchUrl($sSrchUrl, $this->pblk_name); // pblk_name
		$this->BuildSearchUrl($sSrchUrl, $this->pblk_bgcolor); // pblk_bgcolor
		$this->BuildSearchUrl($sSrchUrl, $this->pblk_stext); // pblk_stext
	...
	function LoadSearchValues() {
		...
		// pblk_order
		$this->pblk_order->AdvancedSearch->SearchValue = $objForm->GetValue("x_pblk_order");
		$this->pblk_order->AdvancedSearch->SearchOperator = $objForm->GetValue("z_pblk_order");

		// pblk_status
		$this->pblk_status->AdvancedSearch->SearchValue = $objForm->GetValue("x_pblk_status");
		$this->pblk_status->AdvancedSearch->SearchOperator = $objForm->GetValue("z_pblk_status");

		// pblk_name
		$this->pblk_name->AdvancedSearch->SearchValue = $objForm->GetValue("x_pblk_name");
		$this->pblk_name->AdvancedSearch->SearchOperator = $objForm->GetValue("z_pblk_name");

		// pblk_bgcolor
		$this->pblk_bgcolor->AdvancedSearch->SearchValue = $objForm->GetValue("x_pblk_bgcolor");
		$this->pblk_bgcolor->AdvancedSearch->SearchOperator = $objForm->GetValue("z_pblk_bgcolor");

		// pblk_stext
		$this->pblk_stext->AdvancedSearch->SearchValue = $objForm->GetValue("x_pblk_stext");
		$this->pblk_stext->AdvancedSearch->SearchOperator = $objForm->GetValue("z_pblk_stext");
	}
	...
	function RenderRow() {
		...
		$this->pblk_stext->ViewCustomAttributes = "";
			...
			if ($rswrk && !$rswrk->EOF) { // Lookup values found
				...
				$this->blk_id->AdvancedSearch->ViewValue = $this->blk_id->DisplayValue($arwrk);
			} else {
				$this->blk_id->AdvancedSearch->ViewValue = $Language->Phrase("PleaseSelect");
			}
			$arwrk = ($rswrk) ? $rswrk->GetRows() : array();
			if ($rswrk) $rswrk->Close();
			...
			$this->pblk_order->EditValue = ew_HtmlEncode($this->pblk_order->AdvancedSearch->SearchValue);
			...
			$this->pblk_name->EditValue = ew_HtmlEncode($this->pblk_name->AdvancedSearch->SearchValue);
			...
			$this->pblk_stext->EditValue = ew_HtmlEncode($this->pblk_stext->AdvancedSearch->SearchValue);
	...
	function ValidateSearch() {
		...
		if (!EW_SERVER_VALIDATE)
			return TRUE;
		if (!ew_CheckInteger($this->pblk_order->AdvancedSearch->SearchValue)) {
			ew_AddMessage($gsSearchError, $this->pblk_order->FldErrMsg());
		}
...
// Create page object
if (!isset($cpy_page_block_search)) $cpy_page_block_search = new ccpy_page_block_search();

// Page init
$cpy_page_block_search->Page_Init();

// Page main
$cpy_page_block_search->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$cpy_page_block_search->Page_Render();
?>
...
<?php if ($cpy_page_block_search->IsModal) { ?>
var CurrentAdvancedSearchForm = fcpy_page_blocksearch = new ew_Form("fcpy_page_blocksearch", "search");
<?php } else { ?>
var CurrentForm = fcpy_page_blocksearch = new ew_Form("fcpy_page_blocksearch", "search");
<?php } ?>

// Form_CustomValidate event
fcpy_page_blocksearch.Form_CustomValidate = 
...
fcpy_page_blocksearch.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fcpy_page_blocksearch.Lists["x_page_id"] = {"LinkField":"x_page_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_page_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"cpy_page"};
fcpy_page_blocksearch.Lists["x_page_id"].Data = "<?php echo $cpy_page_block_search->page_id->LookupFilterQuery(FALSE, "search") ?>";
fcpy_page_blocksearch.Lists["x_blk_id"] = {"LinkField":"x_blk_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_blk_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"cpy_block"};
fcpy_page_blocksearch.Lists["x_blk_id"].Data = "<?php echo $cpy_page_block_search->blk_id->LookupFilterQuery(FALSE, "search") ?>";
fcpy_page_blocksearch.Lists["x_pblk_status"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcpy_page_blocksearch.Lists["x_pblk_status"].Options = <?php echo json_encode($cpy_page_block_search->pblk_status->Options()) ?>;
fcpy_page_blocksearch.Lists["x_pblk_bgcolor"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcpy_page_blocksearch.Lists["x_pblk_bgcolor"].Options = <?php echo json_encode($cpy_page_block_search->pblk_bgcolor->Options()) ?>;

// Form object for search
// Validate function for search

fcpy_page_blocksearch.Validate = function(fobj) {
...
<?php $cpy_page_block_search->ShowPageHeader(); ?>
<?php
$cpy_page_block_search->ShowMessage();
?>
<form name="fcpy_page_blocksearch" id="fcpy_page_blocksearch" class="<?php echo $cpy_page_block_search->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($cpy_page_block_search->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $cpy_page_block_search->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="cpy_page_block">
<input type="hidden" name="a_search" id="a_search" value="S">
<input type="hidden" name="modal" value="<?php echo intval($cpy_page_block_search->IsModal) ?>">
<div class="ewSearchDiv"><!-- page* -->
<?php if ($cpy_page_block->page_id->Visible) { // page_id ?>
	<div id="r_page_id" class="form-group">
		<label for="x_page_id" class="<?php echo $cpy_page_block_search->LeftColumnClass ?>"><span id="elh_cpy_page_block_page_id"><?php echo $cpy_page_block->page_id->FldCaption() ?></span>
		...
		<div class="<?php echo $cpy_page_block_search->RightColumnClass ?>"><div<?php echo $cpy_page_block->page_id->CellAttributes() ?>>
...
<?php if ($cpy_page_block->blk_id->Visible) { // blk_id ?>
	<div id="r_blk_id" class="form-group">
		<label for="x_blk_id" class="<?php echo $cpy_page_block_search->LeftColumnClass ?>"><span id="elh_cpy_page_block_blk_id"><?php echo $cpy_page_block->blk_id->FldCaption() ?></span>
		...
		<div class="<?php echo $cpy_page_block_search->RightColumnClass ?>"><div<?php echo $cpy_page_block->blk_id->CellAttributes() ?>>
			<span id="el_cpy_page_block_blk_id">
<span class="ewLookupList">
	<span onclick="jQuery(this).parent().next().click();" tabindex="-1" class="form-control ewLookupText" id="lu_x_blk_id"><?php echo (strval($cpy_page_block->blk_id->AdvancedSearch->ViewValue) == "" ? $Language->Phrase("PleaseSelect") : $cpy_page_block->blk_id->AdvancedSearch->ViewValue); ?></span>
</span>
...
<input type="hidden" data-table="cpy_page_block" data-field="x_blk_id" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $cpy_page_block->blk_id->DisplayValueSeparatorAttribute() ?>" name="x_blk_id" id="x_blk_id" value="<?php echo $cpy_page_block->blk_id->AdvancedSearch->SearchValue ?>"<?php echo $cpy_page_block->blk_id->EditAttributes() ?>>
</span>
		</div></div>
	</div>
<?php } ?>
<?php if ($cpy_page_block->pblk_order->Visible) { // pblk_order ?>
	<div id="r_pblk_order" class="form-group">
		<label for="x_pblk_order" class="<?php echo $cpy_page_block_search->LeftColumnClass ?>"><span id="elh_cpy_page_block_pblk_order"><?php echo $cpy_page_block->pblk_order->FldCaption() ?></span>
		<p class="form-control-static ewSearchOperator"><?php echo $Language->Phrase("=") ?><input type="hidden" name="z_pblk_order" id="z_pblk_order" value="="></p>
		</label>
		<div class="<?php echo $cpy_page_block_search->RightColumnClass ?>"><div<?php echo $cpy_page_block->pblk_order->CellAttributes() ?>>
...
<?php if ($cpy_page_block->pblk_status->Visible) { // pblk_status ?>
	<div id="r_pblk_status" class="form-group">
		<label for="x_pblk_status" class="<?php echo $cpy_page_block_search->LeftColumnClass ?>"><span id="elh_cpy_page_block_pblk_status"><?php echo $cpy_page_block->pblk_status->FldCaption() ?></span>
		<p class="form-control-static ewSearchOperator"><?php echo $Language->Phrase("=") ?><input type="hidden" name="z_pblk_status" id="z_pblk_status" value="="></p>
		</label>
		<div class="<?php echo $cpy_page_block_search->RightColumnClass ?>"><div<?php echo $cpy_page_block->pblk_status->CellAttributes() ?>>
...
<?php if ($cpy_page_block->pblk_name->Visible) { // pblk_name ?>
	<div id="r_pblk_name" class="form-group">
		<label for="x_pblk_name" class="<?php echo $cpy_page_block_search->LeftColumnClass ?>"><span id="elh_cpy_page_block_pblk_name"><?php echo $cpy_page_block->pblk_name->FldCaption() ?></span>
		<p class="form-control-static ewSearchOperator"><?php echo $Language->Phrase("LIKE") ?><input type="hidden" name="z_pblk_name" id="z_pblk_name" value="LIKE"></p>
		</label>
		<div class="<?php echo $cpy_page_block_search->RightColumnClass ?>"><div<?php echo $cpy_page_block->pblk_name->CellAttributes() ?>>
...
<?php if ($cpy_page_block->pblk_bgcolor->Visible) { // pblk_bgcolor ?>
	<div id="r_pblk_bgcolor" class="form-group">
		<label for="x_pblk_bgcolor" class="<?php echo $cpy_page_block_search->LeftColumnClass ?>"><span id="elh_cpy_page_block_pblk_bgcolor"><?php echo $cpy_page_block->pblk_bgcolor->FldCaption() ?></span>
		<p class="form-control-static ewSearchOperator"><?php echo $Language->Phrase("LIKE") ?><input type="hidden" name="z_pblk_bgcolor" id="z_pblk_bgcolor" value="LIKE"></p>
		</label>
		<div class="<?php echo $cpy_page_block_search->RightColumnClass ?>"><div<?php echo $cpy_page_block->pblk_bgcolor->CellAttributes() ?>>
...
<?php if ($cpy_page_block->pblk_stext->Visible) { // pblk_stext ?>
	<div id="r_pblk_stext" class="form-group">
		<label for="x_pblk_stext" class="<?php echo $cpy_page_block_search->LeftColumnClass ?>"><span id="elh_cpy_page_block_pblk_stext"><?php echo $cpy_page_block->pblk_stext->FldCaption() ?></span>
		<p class="form-control-static ewSearchOperator"><?php echo $Language->Phrase("LIKE") ?><input type="hidden" name="z_pblk_stext" id="z_pblk_stext" value="LIKE"></p>
		</label>
		<div class="<?php echo $cpy_page_block_search->RightColumnClass ?>"><div<?php echo $cpy_page_block->pblk_stext->CellAttributes() ?>>
			<span id="el_cpy_page_block_pblk_stext">
<input type="text" data-table="cpy_page_block" data-field="x_pblk_stext" name="x_pblk_stext" id="x_pblk_stext" size="35" placeholder="<?php echo ew_HtmlEncode($cpy_page_block->pblk_stext->getPlaceHolder()) ?>" value="<?php echo $cpy_page_block->pblk_stext->EditValue ?>"<?php echo $cpy_page_block->pblk_stext->EditAttributes() ?>>
</span>
		</div></div>
	</div>
<?php } ?>
...
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $cpy_page_block_search->OffsetColumnClass ?>"><!-- buttons offset -->
...
<script type="text/javascript">
fcpy_page_blocksearch.Init();
</script>
<?php
$cpy_page_block_search->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$cpy_page_block_search->Page_Terminate();
?>
