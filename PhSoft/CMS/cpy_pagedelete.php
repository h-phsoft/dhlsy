...
$cpy_page_delete = NULL; // Initialize page object first

class ccpy_page_delete extends ccpy_page {
	...
	// Page object name
	var $PageObjName = 'cpy_page_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("cpy_pagelist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in cpy_page class, cpy_pageinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("cpy_pagelist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($cpy_page_delete)) $cpy_page_delete = new ccpy_page_delete();

// Page init
$cpy_page_delete->Page_Init();

// Page main
$cpy_page_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$cpy_page_delete->Page_Render();
?>
...
var CurrentForm = fcpy_pagedelete = new ew_Form("fcpy_pagedelete", "delete");

// Form_CustomValidate event
fcpy_pagedelete.Form_CustomValidate = 
...
fcpy_pagedelete.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fcpy_pagedelete.Lists["x_slid_id"] = {"LinkField":"x_slid_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_slid_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"cpy_slider_mst"};
fcpy_pagedelete.Lists["x_slid_id"].Data = "<?php echo $cpy_page_delete->slid_id->LookupFilterQuery(FALSE, "delete") ?>";
...
<?php $cpy_page_delete->ShowPageHeader(); ?>
<?php
$cpy_page_delete->ShowMessage();
?>
<form name="fcpy_pagedelete" id="fcpy_pagedelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($cpy_page_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $cpy_page_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="cpy_page">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($cpy_page_delete->RecKeys as $key) { ?>
...
<?php if ($cpy_page->page_name->Visible) { // page_name ?>
		<th class="<?php echo $cpy_page->page_name->HeaderCellClass() ?>"><span id="elh_cpy_page_page_name" class="cpy_page_page_name"><?php echo $cpy_page->page_name->FldCaption() ?></span></th>
<?php } ?>
<?php if ($cpy_page->page_status->Visible) { // page_status ?>
		<th class="<?php echo $cpy_page->page_status->HeaderCellClass() ?>"><span id="elh_cpy_page_page_status" class="cpy_page_page_status"><?php echo $cpy_page->page_status->FldCaption() ?></span></th>
<?php } ?>
<?php if ($cpy_page->slid_id->Visible) { // slid_id ?>
		<th class="<?php echo $cpy_page->slid_id->HeaderCellClass() ?>"><span id="elh_cpy_page_slid_id" class="cpy_page_slid_id"><?php echo $cpy_page->slid_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($cpy_page->page_stext->Visible) { // page_stext ?>
		<th class="<?php echo $cpy_page->page_stext->HeaderCellClass() ?>"><span id="elh_cpy_page_page_stext" class="cpy_page_page_stext"><?php echo $cpy_page->page_stext->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$cpy_page_delete->RecCnt = 0;
$i = 0;
while (!$cpy_page_delete->Recordset->EOF) {
	$cpy_page_delete->RecCnt++;
	$cpy_page_delete->RowCnt++;

	// Set row properties
	$cpy_page->ResetAttrs();
	$cpy_page->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$cpy_page_delete->LoadRowValues($cpy_page_delete->Recordset);

	// Render row
	$cpy_page_delete->RenderRow();
?>
	<tr<?php echo $cpy_page->RowAttributes() ?>>
<?php if ($cpy_page->page_name->Visible) { // page_name ?>
		<td<?php echo $cpy_page->page_name->CellAttributes() ?>>
<span id="el<?php echo $cpy_page_delete->RowCnt ?>_cpy_page_page_name" class="cpy_page_page_name">
...
<span id="el<?php echo $cpy_page_delete->RowCnt ?>_cpy_page_page_status" class="cpy_page_page_status">
...
<span id="el<?php echo $cpy_page_delete->RowCnt ?>_cpy_page_slid_id" class="cpy_page_slid_id">
...
<span id="el<?php echo $cpy_page_delete->RowCnt ?>_cpy_page_page_stext" class="cpy_page_page_stext">
...
<?php
	$cpy_page_delete->Recordset->MoveNext();
}
$cpy_page_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $cpy_page_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fcpy_pagedelete.Init();
</script>
<?php
$cpy_page_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$cpy_page_delete->Page_Terminate();
?>
