...
//
// Table class for cpy_page
//
class ccpy_page extends cTable {
	var $page_id;
	var $page_name;
	var $page_status;
	var $slid_id;
	...
	function __construct() {
		...
		$this->TableVar = 'cpy_page';
		$this->TableName = 'cpy_page';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`cpy_page`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// page_id
		$this->page_id = new cField('cpy_page', 'cpy_page', 'x_page_id', 'page_id', '`page_id`', '`page_id`', 3, -1, FALSE, '`page_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->page_id->Sortable = FALSE; // Allow sort
		$this->page_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['page_id'] = &$this->page_id;

		// page_name
		$this->page_name = new cField('cpy_page', 'cpy_page', 'x_page_name', 'page_name', '`page_name`', '`page_name`', 200, -1, FALSE, '`page_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->page_name->Sortable = TRUE; // Allow sort
		$this->fields['page_name'] = &$this->page_name;

		// page_status
		$this->page_status = new cField('cpy_page', 'cpy_page', 'x_page_status', 'page_status', '`page_status`', '`page_status`', 2, -1, FALSE, '`page_status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->page_status->Sortable = TRUE; // Allow sort
		$this->page_status->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['page_status'] = &$this->page_status;

		// slid_id
		$this->slid_id = new cField('cpy_page', 'cpy_page', 'x_slid_id', 'slid_id', '`slid_id`', '`slid_id`', 3, -1, FALSE, '`slid_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// page_stext
		$this->page_stext = new cField('cpy_page', 'cpy_page', 'x_page_stext', 'page_stext', '`page_stext`', '`page_stext`', 201, -1, FALSE, '`page_stext`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->page_stext->Sortable = TRUE; // Allow sort
		$this->fields['page_stext'] = &$this->page_stext;

		// page_desc
		$this->page_desc = new cField('cpy_page', 'cpy_page', 'x_page_desc', 'page_desc', '`page_desc`', '`page_desc`', 201, -1, FALSE, '`page_desc`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->page_desc->Sortable = TRUE; // Allow sort
		$this->fields['page_desc'] = &$this->page_desc;
	}
	...
	function GetDetailUrl() {
		...
		// Detail url
		$sDetailUrl = "";
		if ($this->getCurrentDetailTable() == "cpy_page_block") {
			$sDetailUrl = $GLOBALS["cpy_page_block"]->GetListUrl() . "?" . EW_TABLE_SHOW_MASTER . "=" . $this->TableVar;
			$sDetailUrl .= "&fk_page_id=" . urlencode($this->page_id->CurrentValue);
		}
		if ($sDetailUrl == "") {
			$sDetailUrl = "cpy_pagelist.php";
		}
		return $sDetailUrl;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`cpy_page`";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "cpy_pageedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "cpy_pageadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("cpy_pageview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("cpy_pageview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "cpy_pageadd.php?" . $this->UrlParm($parm);
		else
			$url = "cpy_pageadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("cpy_pageedit.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("cpy_pageedit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("cpy_pageadd.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("cpy_pageadd.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("cpy_pagedelete.php", $this->UrlParm());
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->page_status->setDbValue($rs->fields('page_status'));
		$this->slid_id->setDbValue($rs->fields('slid_id'));
		$this->page_stext->setDbValue($rs->fields('page_stext'));
		$this->page_desc->setDbValue($rs->fields('page_desc'));
	}
	...
	function RenderEditRow() {
		...
		$this->page_status->EditValue = $this->page_status->CurrentValue;
		...
		$this->page_stext->EditValue = $this->page_stext->CurrentValue;
		...
		$this->page_desc->EditValue = $this->page_desc->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->page_status->Exportable) $Doc->ExportCaption($this->page_status);
					if ($this->slid_id->Exportable) $Doc->ExportCaption($this->slid_id);
					if ($this->page_stext->Exportable) $Doc->ExportCaption($this->page_stext);
					if ($this->page_desc->Exportable) $Doc->ExportCaption($this->page_desc);
				} else {
					if ($this->page_name->Exportable) $Doc->ExportCaption($this->page_name);
					if ($this->page_status->Exportable) $Doc->ExportCaption($this->page_status);
					if ($this->slid_id->Exportable) $Doc->ExportCaption($this->slid_id);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->page_status->Exportable) $Doc->ExportField($this->page_status);
						if ($this->slid_id->Exportable) $Doc->ExportField($this->slid_id);
						if ($this->page_stext->Exportable) $Doc->ExportField($this->page_stext);
						if ($this->page_desc->Exportable) $Doc->ExportField($this->page_desc);
					} else {
						if ($this->page_name->Exportable) $Doc->ExportField($this->page_name);
						if ($this->page_status->Exportable) $Doc->ExportField($this->page_status);
						if ($this->slid_id->Exportable) $Doc->ExportField($this->slid_id);
