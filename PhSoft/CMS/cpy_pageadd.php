...
class ccpy_page_add extends ccpy_page {
	...
	function ValidateForm() {
		...
		if (in_array("cpy_page_block", $DetailTblVar) && $GLOBALS["cpy_page_block"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("cpy_page_block", $DetailTblVar) && $GLOBALS["cpy_page_block"]->DetailAdd) {
				$GLOBALS["cpy_page_block"]->page_id->setSessionValue($this->page_id->CurrentValue); // Set master key
				if (!isset($GLOBALS["cpy_page_block_grid"])) $GLOBALS["cpy_page_block_grid"] = new ccpy_page_block_grid(); // Get detail page object
				$Security->LoadCurrentUserLevel($this->ProjectID . "cpy_page_block"); // Load user level of detail table
				$AddRow = $GLOBALS["cpy_page_block_grid"]->GridInsert();
				$Security->LoadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
				if (!$AddRow)
					$GLOBALS["cpy_page_block"]->page_id->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetupDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("cpy_page_block", $DetailTblVar)) {
				...
				if ($GLOBALS["cpy_page_block_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["cpy_page_block_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["cpy_page_block_grid"]->CurrentMode = "add";
					$GLOBALS["cpy_page_block_grid"]->CurrentAction = "gridadd";
...
<?php if ($cpy_page->page_status->Visible) { // page_status ?>
	<div id="r_page_status" class="form-group">
		<label id="elh_cpy_page_page_status" for="x_page_status" class="<?php echo $cpy_page_add->LeftColumnClass ?>"><?php echo $cpy_page->page_status->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $cpy_page_add->RightColumnClass ?>"><div<?php echo $cpy_page->page_status->CellAttributes() ?>>
...
<?php if ($cpy_page->slid_id->Visible) { // slid_id ?>
	<div id="r_slid_id" class="form-group">
		<label id="elh_cpy_page_slid_id" for="x_slid_id" class="<?php echo $cpy_page_add->LeftColumnClass ?>"><?php echo $cpy_page->slid_id->FldCaption() ?></label>
...
<?php if ($cpy_page->page_desc->Visible) { // page_desc ?>
	<div id="r_page_desc" class="form-group">
		<label id="elh_cpy_page_page_desc" class="<?php echo $cpy_page_add->LeftColumnClass ?>"><?php echo $cpy_page->page_desc->FldCaption() ?></label>
...
<script type="text/javascript">
ew_CreateEditor("fcpy_pageadd", "x_page_desc", 35, 4, <?php echo ($cpy_page->page_desc->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<?php
	if (in_array("cpy_page_block", explode(",", $cpy_page->getCurrentDetailTable())) && $cpy_page_block->DetailAdd) {
?>
