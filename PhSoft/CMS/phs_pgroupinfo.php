...
//
// Table class for phs_pgroup
//
class cphs_pgroup extends cTable {
	var $pgrp_id;
	var $pgrp_name;
	...
	function __construct() {
		...
		$this->TableVar = 'phs_pgroup';
		$this->TableName = 'phs_pgroup';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`phs_pgroup`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// pgrp_id
		$this->pgrp_id = new cField('phs_pgroup', 'phs_pgroup', 'x_pgrp_id', 'pgrp_id', '`pgrp_id`', '`pgrp_id`', 3, -1, FALSE, '`pgrp_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->pgrp_id->Sortable = FALSE; // Allow sort
		$this->pgrp_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['pgrp_id'] = &$this->pgrp_id;

		// pgrp_name
		$this->pgrp_name = new cField('phs_pgroup', 'phs_pgroup', 'x_pgrp_name', 'pgrp_name', '`pgrp_name`', '`pgrp_name`', 200, -1, FALSE, '`pgrp_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`phs_pgroup`";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "phs_pgroupedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "phs_pgroupadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("phs_pgroupview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("phs_pgroupview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "phs_pgroupadd.php?" . $this->UrlParm($parm);
		else
			$url = "phs_pgroupadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("phs_pgroupedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("phs_pgroupadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("phs_pgroupdelete.php", $this->UrlParm());
	}
