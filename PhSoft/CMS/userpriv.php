...
class cuserpriv extends cphs_pgroup {
	...
	function __construct() {
		...
		if (!isset($GLOBALS["phs_pgroup"])) $GLOBALS["phs_pgroup"] = &$this;
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'phs_pgroup');
	...
	function Page_Main() {
		...
		$Breadcrumb->Add("list", "phs_pgroup", "phs_pgrouplist.php", "", "phs_pgroup");
		...
		if (@$_POST["a_edit"] == "") {
			...
			if ($this->pgrp_id->QueryStringValue == "-1") {
			...
			$this->pgrp_id->setFormValue($_POST["x_pgrp_id"]);
		...
		switch ($this->CurrentAction) {
			case "I": // Display
				...
				$ar = array();
				for ($i = 0; $i < $this->TableNameCount; $i++) {
					$tempPriv = $Security->GetUserLevelPrivEx($this->TableList[$i][4] . $this->TableList[$i][0], $this->pgrp_id->CurrentValue);
	...
	function EditRow() {
		...
		foreach ($this->Privileges as $i => $privilege) {
			$Sql = "SELECT * FROM " . EW_USER_LEVEL_PRIV_TABLE . " WHERE " .
				...
				EW_USER_LEVEL_PRIV_USER_LEVEL_ID_FIELD . " = " . $this->pgrp_id->CurrentValue;
			$rs = $c->Execute($Sql);
			if ($rs && !$rs->EOF) {
				...
				$Sql = "INSERT INTO " . EW_USER_LEVEL_PRIV_TABLE . " (" . EW_USER_LEVEL_PRIV_TABLE_NAME_FIELD . ", " . EW_USER_LEVEL_PRIV_USER_LEVEL_ID_FIELD . ", " . EW_USER_LEVEL_PRIV_PRIV_FIELD . ") VALUES ('" . ew_AdjustSql($this->TableList[$i][4] . $this->TableList[$i][0], EW_USER_LEVEL_PRIV_DBID) . "', " . $this->pgrp_id->CurrentValue . ", " . $privilege . ")";
				$c->Execute($Sql);
			}
			if ($rs)
				$rs->Close();
...
<input type="hidden" name="x_pgrp_id" id="x_pgrp_id" value="<?php echo $phs_pgroup->pgrp_id->CurrentValue ?>">
<div class="ewDesktop">
<div class="box ewBox ewGrid">
<div class="box-header with-border">
	<h3 class="box-title"><?php echo $Language->Phrase("UserLevel") ?><?php echo $Security->GetUserLevelName($phs_pgroup->pgrp_id->CurrentValue) ?> (<?php echo $phs_pgroup->pgrp_id->CurrentValue ?>)</h3>
