...
$cpy_page_edit = NULL; // Initialize page object first

class ccpy_page_edit extends ccpy_page {
	...
	function Page_Main() {
		...
		if ($this->TotalRecs <= 0) { // No record found
			...
			$this->Page_Terminate("cpy_pagelist.php"); // Return to list page
	...
	function ValidateForm() {
		...
		if (in_array("cpy_page_block", $DetailTblVar) && $GLOBALS["cpy_page_block"]->DetailEdit) {
			if (!isset($GLOBALS["cpy_page_block_grid"])) $GLOBALS["cpy_page_block_grid"] = new ccpy_page_block_grid(); // get detail page object
			$GLOBALS["cpy_page_block_grid"]->ValidateGridForm();
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// page_desc
			$this->page_desc->SetDbValueDef($rsnew, $this->page_desc->CurrentValue, NULL, $this->page_desc->ReadOnly);
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					if (in_array("cpy_page_block", $DetailTblVar) && $GLOBALS["cpy_page_block"]->DetailEdit) {
						if (!isset($GLOBALS["cpy_page_block_grid"])) $GLOBALS["cpy_page_block_grid"] = new ccpy_page_block_grid(); // Get detail page object
						$Security->LoadCurrentUserLevel($this->ProjectID . "cpy_page_block"); // Load user level of detail table
						$EditRow = $GLOBALS["cpy_page_block_grid"]->GridUpdate();
	...
	function SetupDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("cpy_page_block", $DetailTblVar)) {
				if (!isset($GLOBALS["cpy_page_block_grid"]))
					$GLOBALS["cpy_page_block_grid"] = new ccpy_page_block_grid;
				if ($GLOBALS["cpy_page_block_grid"]->DetailEdit) {
					$GLOBALS["cpy_page_block_grid"]->CurrentMode = "edit";
					$GLOBALS["cpy_page_block_grid"]->CurrentAction = "gridedit";

					// Save current master table to detail table
					$GLOBALS["cpy_page_block_grid"]->setCurrentMasterTable($this->TableVar);
					$GLOBALS["cpy_page_block_grid"]->setStartRecordNumber(1);
					$GLOBALS["cpy_page_block_grid"]->page_id->FldIsDetailKey = TRUE;
					$GLOBALS["cpy_page_block_grid"]->page_id->CurrentValue = $this->page_id->CurrentValue;
					$GLOBALS["cpy_page_block_grid"]->page_id->setSessionValue($GLOBALS["cpy_page_block_grid"]->page_id->CurrentValue);
				}
			}
		}
	}
...
// Create page object
if (!isset($cpy_page_edit)) $cpy_page_edit = new ccpy_page_edit();

// Page init
$cpy_page_edit->Page_Init();

// Page main
$cpy_page_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$cpy_page_edit->Page_Render();
?>
...
var CurrentForm = fcpy_pageedit = new ew_Form("fcpy_pageedit", "edit");

// Validate form
fcpy_pageedit.Validate = function() {
...
fcpy_pageedit.Form_CustomValidate = 
...
fcpy_pageedit.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fcpy_pageedit.Lists["x_slid_id"] = {"LinkField":"x_slid_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_slid_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"cpy_slider_mst"};
fcpy_pageedit.Lists["x_slid_id"].Data = "<?php echo $cpy_page_edit->slid_id->LookupFilterQuery(FALSE, "edit") ?>";
...
<?php $cpy_page_edit->ShowPageHeader(); ?>
<?php
$cpy_page_edit->ShowMessage();
?>
<?php if (!$cpy_page_edit->IsModal) { ?>
<form name="ewPagerForm" class="form-horizontal ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($cpy_page_edit->Pager)) $cpy_page_edit->Pager = new cPrevNextPager($cpy_page_edit->StartRec, $cpy_page_edit->DisplayRecs, $cpy_page_edit->TotalRecs, $cpy_page_edit->AutoHidePager) ?>
<?php if ($cpy_page_edit->Pager->RecordCount > 0 && $cpy_page_edit->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($cpy_page_edit->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $cpy_page_edit->PageUrl() ?>start=<?php echo $cpy_page_edit->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($cpy_page_edit->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $cpy_page_edit->PageUrl() ?>start=<?php echo $cpy_page_edit->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $cpy_page_edit->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($cpy_page_edit->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $cpy_page_edit->PageUrl() ?>start=<?php echo $cpy_page_edit->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($cpy_page_edit->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $cpy_page_edit->PageUrl() ?>start=<?php echo $cpy_page_edit->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $cpy_page_edit->Pager->PageCount ?></span>
</div>
<?php } ?>
<div class="clearfix"></div>
</form>
<?php } ?>
<form name="fcpy_pageedit" id="fcpy_pageedit" class="<?php echo $cpy_page_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($cpy_page_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $cpy_page_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="cpy_page">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<input type="hidden" name="modal" value="<?php echo intval($cpy_page_edit->IsModal) ?>">
<div class="ewEditDiv"><!-- page* -->
<?php if ($cpy_page->page_name->Visible) { // page_name ?>
	<div id="r_page_name" class="form-group">
		<label id="elh_cpy_page_page_name" for="x_page_name" class="<?php echo $cpy_page_edit->LeftColumnClass ?>"><?php echo $cpy_page->page_name->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $cpy_page_edit->RightColumnClass ?>"><div<?php echo $cpy_page->page_name->CellAttributes() ?>>
<span id="el_cpy_page_page_name">
<input type="text" data-table="cpy_page" data-field="x_page_name" name="x_page_name" id="x_page_name" placeholder="<?php echo ew_HtmlEncode($cpy_page->page_name->getPlaceHolder()) ?>" value="<?php echo $cpy_page->page_name->EditValue ?>"<?php echo $cpy_page->page_name->EditAttributes() ?>>
</span>
<?php echo $cpy_page->page_name->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($cpy_page->page_status->Visible) { // page_status ?>
	<div id="r_page_status" class="form-group">
		<label id="elh_cpy_page_page_status" for="x_page_status" class="<?php echo $cpy_page_edit->LeftColumnClass ?>"><?php echo $cpy_page->page_status->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $cpy_page_edit->RightColumnClass ?>"><div<?php echo $cpy_page->page_status->CellAttributes() ?>>
<span id="el_cpy_page_page_status">
<input type="text" data-table="cpy_page" data-field="x_page_status" name="x_page_status" id="x_page_status" size="30" placeholder="<?php echo ew_HtmlEncode($cpy_page->page_status->getPlaceHolder()) ?>" value="<?php echo $cpy_page->page_status->EditValue ?>"<?php echo $cpy_page->page_status->EditAttributes() ?>>
</span>
<?php echo $cpy_page->page_status->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($cpy_page->slid_id->Visible) { // slid_id ?>
	<div id="r_slid_id" class="form-group">
		<label id="elh_cpy_page_slid_id" for="x_slid_id" class="<?php echo $cpy_page_edit->LeftColumnClass ?>"><?php echo $cpy_page->slid_id->FldCaption() ?></label>
		<div class="<?php echo $cpy_page_edit->RightColumnClass ?>"><div<?php echo $cpy_page->slid_id->CellAttributes() ?>>
...
<?php if ($cpy_page->page_stext->Visible) { // page_stext ?>
	<div id="r_page_stext" class="form-group">
		<label id="elh_cpy_page_page_stext" for="x_page_stext" class="<?php echo $cpy_page_edit->LeftColumnClass ?>"><?php echo $cpy_page->page_stext->FldCaption() ?></label>
		<div class="<?php echo $cpy_page_edit->RightColumnClass ?>"><div<?php echo $cpy_page->page_stext->CellAttributes() ?>>
...
<?php if ($cpy_page->page_desc->Visible) { // page_desc ?>
	<div id="r_page_desc" class="form-group">
		<label id="elh_cpy_page_page_desc" class="<?php echo $cpy_page_edit->LeftColumnClass ?>"><?php echo $cpy_page->page_desc->FldCaption() ?></label>
		<div class="<?php echo $cpy_page_edit->RightColumnClass ?>"><div<?php echo $cpy_page->page_desc->CellAttributes() ?>>
<span id="el_cpy_page_page_desc">
<?php ew_AppendClass($cpy_page->page_desc->EditAttrs["class"], "editor"); ?>
...
<script type="text/javascript">
ew_CreateEditor("fcpy_pageedit", "x_page_desc", 35, 4, <?php echo ($cpy_page->page_desc->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<input type="hidden" data-table="cpy_page" data-field="x_page_id" name="x_page_id" id="x_page_id" value="<?php echo ew_HtmlEncode($cpy_page->page_id->CurrentValue) ?>">
<?php
	if (in_array("cpy_page_block", explode(",", $cpy_page->getCurrentDetailTable())) && $cpy_page_block->DetailEdit) {
?>
<?php if ($cpy_page->getCurrentDetailTable() <> "") { ?>
<h4 class="ewDetailCaption"><?php echo $Language->TablePhrase("cpy_page_block", "TblCaption") ?></h4>
<?php } ?>
<?php include_once "cpy_page_blockgrid.php" ?>
<?php } ?>
<?php if (!$cpy_page_edit->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $cpy_page_edit->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $cpy_page_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
