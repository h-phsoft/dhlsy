...
//
// Table class for phs_users
//
class cphs_users extends cTable {
	var $user_id;
	var $pgrp_id;
	var $user_logon;
	var $user_password;
	...
	function __construct() {
		...
		$this->TableVar = 'phs_users';
		$this->TableName = 'phs_users';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`phs_users`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// user_id
		$this->user_id = new cField('phs_users', 'phs_users', 'x_user_id', 'user_id', '`user_id`', '`user_id`', 3, -1, FALSE, '`user_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->user_id->Sortable = TRUE; // Allow sort
		$this->user_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['user_id'] = &$this->user_id;

		// pgrp_id
		$this->pgrp_id = new cField('phs_users', 'phs_users', 'x_pgrp_id', 'pgrp_id', '`pgrp_id`', '`pgrp_id`', 3, -1, FALSE, '`pgrp_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// user_logon
		$this->user_logon = new cField('phs_users', 'phs_users', 'x_user_logon', 'user_logon', '`user_logon`', '`user_logon`', 200, -1, FALSE, '`user_logon`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->user_logon->Sortable = TRUE; // Allow sort
		$this->fields['user_logon'] = &$this->user_logon;

		// user_password
		$this->user_password = new cField('phs_users', 'phs_users', 'x_user_password', 'user_password', '`user_password`', '`user_password`', 200, -1, FALSE, '`user_password`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'PASSWORD');
		$this->user_password->Sortable = TRUE; // Allow sort
		$this->fields['user_password'] = &$this->user_password;

		// user_email
		$this->user_email = new cField('phs_users', 'phs_users', 'x_user_email', 'user_email', '`user_email`', '`user_email`', 200, -1, FALSE, '`user_email`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`phs_users`";
	}
	...
	function getSqlOrderBy() { // Order By
		return ($this->_SqlOrderBy <> "") ? $this->_SqlOrderBy : "`pgrp_id` ASC,`user_logon` ASC";
	}
	...
	function InsertSQL(&$rs) {
		...
		foreach ($rs as $name => $value) {
			if (!isset($this->fields[$name]) || $this->fields[$name]->FldIsCustom)
				continue;
			if (EW_ENCRYPTED_PASSWORD && $name == 'user_password')
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "phs_usersedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "phs_usersadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("phs_usersview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("phs_usersview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "phs_usersadd.php?" . $this->UrlParm($parm);
		else
			$url = "phs_usersadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("phs_usersedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("phs_usersadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("phs_usersdelete.php", $this->UrlParm());
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->user_logon->setDbValue($rs->fields('user_logon'));
	...
	function RenderEditRow() {
		...
		$this->user_logon->EditValue = $this->user_logon->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->user_logon->Exportable) $Doc->ExportCaption($this->user_logon);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->user_logon->Exportable) $Doc->ExportField($this->user_logon);
