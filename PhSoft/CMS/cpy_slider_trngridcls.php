...
$cpy_slider_trn_grid = NULL; // Initialize page object first

class ccpy_slider_trn_grid extends ccpy_slider_trn {
	...
	// Page object name
	var $PageObjName = 'cpy_slider_trn_grid';

	// Grid form hidden field names
	var $FormName = 'fcpy_slider_trngrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["cpy_slider_trn"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function LoadDefaultValues() {
		...
		$this->slid_photo->Upload->Index = $this->RowIndex;
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->tslid_id->CurrentValue = strval($arKeys[0]); // tslid_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetupMasterParms() {
		...
		if ($sMasterTblVar == "cpy_slider_mst") {
			$this->slid_id->Visible = FALSE;
			if ($GLOBALS["cpy_slider_mst"]->EventCancelled) $this->EventCancelled = TRUE;
		}
