...
fphs_userssearch.Validate = function(fobj) {
	...
	if (elm && !ew_CheckInteger(elm.value))
		return this.OnError(elm, "<?php echo ew_JsEncode2($phs_users->user_id->FldErrMsg()) ?>");
...
<?php if ($phs_users->user_id->Visible) { // user_id ?>
	<div id="r_user_id" class="form-group">
		<label for="x_user_id" class="<?php echo $phs_users_search->LeftColumnClass ?>"><span id="elh_phs_users_user_id"><?php echo $phs_users->user_id->FldCaption() ?></span>
		...
		<div class="<?php echo $phs_users_search->RightColumnClass ?>"><div<?php echo $phs_users->user_id->CellAttributes() ?>>
			<span id="el_phs_users_user_id">
<input type="text" data-table="phs_users" data-field="x_user_id" name="x_user_id" id="x_user_id" placeholder="<?php echo ew_HtmlEncode($phs_users->user_id->getPlaceHolder()) ?>" value="<?php echo $phs_users->user_id->EditValue ?>"<?php echo $phs_users->user_id->EditAttributes() ?>>
</span>
		</div></div>
	</div>
<?php } ?>
