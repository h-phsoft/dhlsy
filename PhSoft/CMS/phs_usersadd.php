...
$phs_users_add = NULL; // Initialize page object first

class cphs_users_add extends cphs_users {
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$loaded) { // Record not loaded
					...
					$this->Page_Terminate("phs_userslist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "phs_usersview.php")
...
// Create page object
if (!isset($phs_users_add)) $phs_users_add = new cphs_users_add();

// Page init
$phs_users_add->Page_Init();

// Page main
$phs_users_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$phs_users_add->Page_Render();
?>
...
var CurrentForm = fphs_usersadd = new ew_Form("fphs_usersadd", "add");

// Validate form
fphs_usersadd.Validate = function() {
...
fphs_usersadd.Form_CustomValidate = 
...
fphs_usersadd.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fphs_usersadd.Lists["x_pgrp_id"] = {"LinkField":"x_pgrp_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_pgrp_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"phs_pgroup"};
fphs_usersadd.Lists["x_pgrp_id"].Data = "<?php echo $phs_users_add->pgrp_id->LookupFilterQuery(FALSE, "add") ?>";
...
<?php $phs_users_add->ShowPageHeader(); ?>
<?php
$phs_users_add->ShowMessage();
?>
<form name="fphs_usersadd" id="fphs_usersadd" class="<?php echo $phs_users_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($phs_users_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $phs_users_add->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="phs_users">
<input type="hidden" name="a_add" id="a_add" value="A">
<input type="hidden" name="modal" value="<?php echo intval($phs_users_add->IsModal) ?>">
...
<?php if ($phs_users->pgrp_id->Visible) { // pgrp_id ?>
	<div id="r_pgrp_id" class="form-group">
		<label id="elh_phs_users_pgrp_id" for="x_pgrp_id" class="<?php echo $phs_users_add->LeftColumnClass ?>"><?php echo $phs_users->pgrp_id->FldCaption() ?></label>
		<div class="<?php echo $phs_users_add->RightColumnClass ?>"><div<?php echo $phs_users->pgrp_id->CellAttributes() ?>>
...
<?php if ($phs_users->user_logon->Visible) { // user_logon ?>
	<div id="r_user_logon" class="form-group">
		<label id="elh_phs_users_user_logon" for="x_user_logon" class="<?php echo $phs_users_add->LeftColumnClass ?>"><?php echo $phs_users->user_logon->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $phs_users_add->RightColumnClass ?>"><div<?php echo $phs_users->user_logon->CellAttributes() ?>>
...
<?php if ($phs_users->user_password->Visible) { // user_password ?>
	<div id="r_user_password" class="form-group">
		<label id="elh_phs_users_user_password" for="x_user_password" class="<?php echo $phs_users_add->LeftColumnClass ?>"><?php echo $phs_users->user_password->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $phs_users_add->RightColumnClass ?>"><div<?php echo $phs_users->user_password->CellAttributes() ?>>
<span id="el_phs_users_user_password">
<input type="password" data-field="x_user_password" name="x_user_password" id="x_user_password" size="30" maxlength="100" placeholder="<?php echo ew_HtmlEncode($phs_users->user_password->getPlaceHolder()) ?>"<?php echo $phs_users->user_password->EditAttributes() ?>>
</span>
...
<?php if ($phs_users->user_email->Visible) { // user_email ?>
	<div id="r_user_email" class="form-group">
		<label id="elh_phs_users_user_email" for="x_user_email" class="<?php echo $phs_users_add->LeftColumnClass ?>"><?php echo $phs_users->user_email->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $phs_users_add->RightColumnClass ?>"><div<?php echo $phs_users->user_email->CellAttributes() ?>>
...
<?php if (!$phs_users_add->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $phs_users_add->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("AddBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $phs_users_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
