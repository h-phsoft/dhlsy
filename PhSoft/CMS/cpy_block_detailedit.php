...
$cpy_block_detail_edit = NULL; // Initialize page object first

class ccpy_block_detail_edit extends ccpy_block_detail {
	...
	// Page object name
	var $PageObjName = 'cpy_block_detail_edit';
	...
	function Page_Main() {
		...
		switch ($this->CurrentAction) {
			...
			Case "U": // Update
				...
				if (ew_GetPageName($sReturnUrl) == "cpy_block_detaillist.php")
	...
	function LoadFormValues() {
		...
		if (!$this->dblk_text->FldIsDetailKey) {
			$this->dblk_text->setFormValue($objForm->GetValue("x_dblk_text"));
		}
		if (!$this->dblk_id->FldIsDetailKey)
			$this->dblk_id->setFormValue($objForm->GetValue("x_dblk_id"));
	}
	...
	function RestoreFormValues() {
		...
		$this->dblk_text->CurrentValue = $this->dblk_text->FormValue;
	}
	...
	function RenderRow() {
		...
		$this->dblk_text->ViewCustomAttributes = "";
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->dblk_image);
			...
			$this->dblk_text->EditValue = ew_HtmlEncode($this->dblk_text->CurrentValue);
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// dblk_text
			$this->dblk_text->SetDbValueDef($rsnew, $this->dblk_text->CurrentValue, NULL, $this->dblk_text->ReadOnly);
...
// Create page object
if (!isset($cpy_block_detail_edit)) $cpy_block_detail_edit = new ccpy_block_detail_edit();

// Page init
$cpy_block_detail_edit->Page_Init();

// Page main
$cpy_block_detail_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$cpy_block_detail_edit->Page_Render();
?>
...
var CurrentForm = fcpy_block_detailedit = new ew_Form("fcpy_block_detailedit", "edit");

// Validate form
fcpy_block_detailedit.Validate = function() {
...
fcpy_block_detailedit.Form_CustomValidate = 
...
fcpy_block_detailedit.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fcpy_block_detailedit.Lists["x_blk_id"] = {"LinkField":"x_blk_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_blk_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"cpy_block"};
fcpy_block_detailedit.Lists["x_blk_id"].Data = "<?php echo $cpy_block_detail_edit->blk_id->LookupFilterQuery(FALSE, "edit") ?>";
fcpy_block_detailedit.Lists["x_dblk_type"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcpy_block_detailedit.Lists["x_dblk_type"].Options = <?php echo json_encode($cpy_block_detail_edit->dblk_type->Options()) ?>;
fcpy_block_detailedit.Lists["x_dblk_status"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcpy_block_detailedit.Lists["x_dblk_status"].Options = <?php echo json_encode($cpy_block_detail_edit->dblk_status->Options()) ?>;
...
<?php $cpy_block_detail_edit->ShowPageHeader(); ?>
<?php
$cpy_block_detail_edit->ShowMessage();
?>
<?php if (!$cpy_block_detail_edit->IsModal) { ?>
<form name="ewPagerForm" class="form-horizontal ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($cpy_block_detail_edit->Pager)) $cpy_block_detail_edit->Pager = new cPrevNextPager($cpy_block_detail_edit->StartRec, $cpy_block_detail_edit->DisplayRecs, $cpy_block_detail_edit->TotalRecs, $cpy_block_detail_edit->AutoHidePager) ?>
<?php if ($cpy_block_detail_edit->Pager->RecordCount > 0 && $cpy_block_detail_edit->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($cpy_block_detail_edit->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $cpy_block_detail_edit->PageUrl() ?>start=<?php echo $cpy_block_detail_edit->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($cpy_block_detail_edit->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $cpy_block_detail_edit->PageUrl() ?>start=<?php echo $cpy_block_detail_edit->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $cpy_block_detail_edit->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($cpy_block_detail_edit->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $cpy_block_detail_edit->PageUrl() ?>start=<?php echo $cpy_block_detail_edit->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($cpy_block_detail_edit->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $cpy_block_detail_edit->PageUrl() ?>start=<?php echo $cpy_block_detail_edit->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $cpy_block_detail_edit->Pager->PageCount ?></span>
</div>
<?php } ?>
<div class="clearfix"></div>
</form>
<?php } ?>
<form name="fcpy_block_detailedit" id="fcpy_block_detailedit" class="<?php echo $cpy_block_detail_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($cpy_block_detail_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $cpy_block_detail_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="cpy_block_detail">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<input type="hidden" name="modal" value="<?php echo intval($cpy_block_detail_edit->IsModal) ?>">
<?php if ($cpy_block_detail->getCurrentMasterTable() == "cpy_block") { ?>
...
<?php if ($cpy_block_detail->blk_id->Visible) { // blk_id ?>
	<div id="r_blk_id" class="form-group">
		<label id="elh_cpy_block_detail_blk_id" for="x_blk_id" class="<?php echo $cpy_block_detail_edit->LeftColumnClass ?>"><?php echo $cpy_block_detail->blk_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $cpy_block_detail_edit->RightColumnClass ?>"><div<?php echo $cpy_block_detail->blk_id->CellAttributes() ?>>
...
<input type="hidden" id="x_blk_id" name="x_blk_id" value="<?php echo ew_HtmlEncode($cpy_block_detail->blk_id->CurrentValue) ?>">
<?php } else { ?>
<span id="el_cpy_block_detail_blk_id">
...
<?php } ?>
<?php echo $cpy_block_detail->blk_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($cpy_block_detail->dblk_order->Visible) { // dblk_order ?>
	<div id="r_dblk_order" class="form-group">
		<label id="elh_cpy_block_detail_dblk_order" for="x_dblk_order" class="<?php echo $cpy_block_detail_edit->LeftColumnClass ?>"><?php echo $cpy_block_detail->dblk_order->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $cpy_block_detail_edit->RightColumnClass ?>"><div<?php echo $cpy_block_detail->dblk_order->CellAttributes() ?>>
...
<?php echo $cpy_block_detail->dblk_order->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($cpy_block_detail->dblk_type->Visible) { // dblk_type ?>
	<div id="r_dblk_type" class="form-group">
		<label id="elh_cpy_block_detail_dblk_type" for="x_dblk_type" class="<?php echo $cpy_block_detail_edit->LeftColumnClass ?>"><?php echo $cpy_block_detail->dblk_type->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $cpy_block_detail_edit->RightColumnClass ?>"><div<?php echo $cpy_block_detail->dblk_type->CellAttributes() ?>>
...
<?php echo $cpy_block_detail->dblk_type->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($cpy_block_detail->dblk_status->Visible) { // dblk_status ?>
	<div id="r_dblk_status" class="form-group">
		<label id="elh_cpy_block_detail_dblk_status" for="x_dblk_status" class="<?php echo $cpy_block_detail_edit->LeftColumnClass ?>"><?php echo $cpy_block_detail->dblk_status->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $cpy_block_detail_edit->RightColumnClass ?>"><div<?php echo $cpy_block_detail->dblk_status->CellAttributes() ?>>
...
<?php echo $cpy_block_detail->dblk_status->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($cpy_block_detail->dblk_name->Visible) { // dblk_name ?>
	<div id="r_dblk_name" class="form-group">
		<label id="elh_cpy_block_detail_dblk_name" for="x_dblk_name" class="<?php echo $cpy_block_detail_edit->LeftColumnClass ?>"><?php echo $cpy_block_detail->dblk_name->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $cpy_block_detail_edit->RightColumnClass ?>"><div<?php echo $cpy_block_detail->dblk_name->CellAttributes() ?>>
...
<?php echo $cpy_block_detail->dblk_name->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($cpy_block_detail->dblk_image->Visible) { // dblk_image ?>
	<div id="r_dblk_image" class="form-group">
		<label id="elh_cpy_block_detail_dblk_image" class="<?php echo $cpy_block_detail_edit->LeftColumnClass ?>"><?php echo $cpy_block_detail->dblk_image->FldCaption() ?></label>
		<div class="<?php echo $cpy_block_detail_edit->RightColumnClass ?>"><div<?php echo $cpy_block_detail->dblk_image->CellAttributes() ?>>
<span id="el_cpy_block_detail_dblk_image">
<div id="fd_x_dblk_image">
<span title="<?php echo $cpy_block_detail->dblk_image->FldTitle() ? $cpy_block_detail->dblk_image->FldTitle() : $Language->Phrase("ChooseFile") ?>" class="btn btn-default btn-sm fileinput-button ewTooltip<?php if ($cpy_block_detail->dblk_image->ReadOnly || $cpy_block_detail->dblk_image->Disabled) echo " hide"; ?>">
	...
	<input type="file" title=" " data-table="cpy_block_detail" data-field="x_dblk_image" name="x_dblk_image" id="x_dblk_image"<?php echo $cpy_block_detail->dblk_image->EditAttributes() ?>>
</span>
<input type="hidden" name="fn_x_dblk_image" id= "fn_x_dblk_image" value="<?php echo $cpy_block_detail->dblk_image->Upload->FileName ?>">
<?php if (@$_POST["fa_x_dblk_image"] == "0") { ?>
<input type="hidden" name="fa_x_dblk_image" id= "fa_x_dblk_image" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_dblk_image" id= "fa_x_dblk_image" value="1">
<?php } ?>
<input type="hidden" name="fs_x_dblk_image" id= "fs_x_dblk_image" value="200">
<input type="hidden" name="fx_x_dblk_image" id= "fx_x_dblk_image" value="<?php echo $cpy_block_detail->dblk_image->UploadAllowedFileExt ?>">
<input type="hidden" name="fm_x_dblk_image" id= "fm_x_dblk_image" value="<?php echo $cpy_block_detail->dblk_image->UploadMaxFileSize ?>">
</div>
<table id="ft_x_dblk_image" class="table table-condensed pull-left ewUploadTable"><tbody class="files"></tbody></table>
</span>
<?php echo $cpy_block_detail->dblk_image->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($cpy_block_detail->dblk_stext->Visible) { // dblk_stext ?>
	<div id="r_dblk_stext" class="form-group">
		<label id="elh_cpy_block_detail_dblk_stext" for="x_dblk_stext" class="<?php echo $cpy_block_detail_edit->LeftColumnClass ?>"><?php echo $cpy_block_detail->dblk_stext->FldCaption() ?></label>
		<div class="<?php echo $cpy_block_detail_edit->RightColumnClass ?>"><div<?php echo $cpy_block_detail->dblk_stext->CellAttributes() ?>>
<span id="el_cpy_block_detail_dblk_stext">
<textarea data-table="cpy_block_detail" data-field="x_dblk_stext" name="x_dblk_stext" id="x_dblk_stext" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($cpy_block_detail->dblk_stext->getPlaceHolder()) ?>"<?php echo $cpy_block_detail->dblk_stext->EditAttributes() ?>><?php echo $cpy_block_detail->dblk_stext->EditValue ?></textarea>
</span>
<?php echo $cpy_block_detail->dblk_stext->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($cpy_block_detail->dblk_text->Visible) { // dblk_text ?>
	<div id="r_dblk_text" class="form-group">
		<label id="elh_cpy_block_detail_dblk_text" class="<?php echo $cpy_block_detail_edit->LeftColumnClass ?>"><?php echo $cpy_block_detail->dblk_text->FldCaption() ?></label>
		<div class="<?php echo $cpy_block_detail_edit->RightColumnClass ?>"><div<?php echo $cpy_block_detail->dblk_text->CellAttributes() ?>>
<span id="el_cpy_block_detail_dblk_text">
<?php ew_AppendClass($cpy_block_detail->dblk_text->EditAttrs["class"], "editor"); ?>
<textarea data-table="cpy_block_detail" data-field="x_dblk_text" name="x_dblk_text" id="x_dblk_text" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($cpy_block_detail->dblk_text->getPlaceHolder()) ?>"<?php echo $cpy_block_detail->dblk_text->EditAttributes() ?>><?php echo $cpy_block_detail->dblk_text->EditValue ?></textarea>
<script type="text/javascript">
ew_CreateEditor("fcpy_block_detailedit", "x_dblk_text", 35, 4, <?php echo ($cpy_block_detail->dblk_text->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
<?php echo $cpy_block_detail->dblk_text->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page* -->
<input type="hidden" data-table="cpy_block_detail" data-field="x_dblk_id" name="x_dblk_id" id="x_dblk_id" value="<?php echo ew_HtmlEncode($cpy_block_detail->dblk_id->CurrentValue) ?>">
<?php if (!$cpy_block_detail_edit->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $cpy_block_detail_edit->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $cpy_block_detail_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
...
<script type="text/javascript">
fcpy_block_detailedit.Init();
</script>
<?php
$cpy_block_detail_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$cpy_block_detail_edit->Page_Terminate();
?>
