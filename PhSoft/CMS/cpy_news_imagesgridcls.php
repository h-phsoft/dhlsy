...
$cpy_news_images_grid = NULL; // Initialize page object first

class ccpy_news_images_grid extends ccpy_news_images {
	...
	// Page object name
	var $PageObjName = 'cpy_news_images_grid';

	// Grid form hidden field names
	var $FormName = 'fcpy_news_imagesgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["cpy_news_images"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function LoadDefaultValues() {
		...
		$this->nimg_photo->Upload->Index = $this->RowIndex;
	}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->nimg_id->CurrentValue = strval($arKeys[0]); // nimg_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetupMasterParms() {
		...
		if ($sMasterTblVar == "cpy_news") {
			...
			if ($GLOBALS["cpy_news"]->EventCancelled) $this->EventCancelled = TRUE;
		}
